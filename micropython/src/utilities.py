...
def goToSleep(quickSleep=False):
    ...
    sleep_duration = max(
        min(config.get("SLEEP_DURATION"), config.get("NEXT_INIT_TIME") - now()), 1
    )
    print(
    ...
    # hold the pin to prevent current leakage.  is this necessary?
    p16 = machine.Pin(config.get("ledPin"), machine.Pin.IN, machine.Pin.PULL_HOLD)

    # sensorVPin = machine.Pin(35, machine.Pin.IN, machine.Pin.PULL_HOLD)
    # sensorGPin = machine.Pin(34, machine.Pin.IN, machine.Pin.PULL_HOLD)
